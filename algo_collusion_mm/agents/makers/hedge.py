...
class MakerHedge(Maker):
    """
    Market-making agent for the GM environment based on the Hedge algorithm.

    This agent maintains a discrete set of possible (ask, bid) strategies and selects one at
    each step according to a probability distribution computed using the Hedge algorithm.
    Hedge balances exploitation of strategies with high historical rewards and exploration
    of others, controlled by the `epsilon` parameter.

    At each step, the agent receives a reward and updates its internal weights for each
    strategy (arm). The rewards are first normalized using a user-provided
    `scale_rewards` function, which maps raw environment rewards into a suitable range
    (typically [0, 1]). This normalization ensures stable updates in the Hedge algorithm
    across different environments.

    Attributes
    ----------
    epsilon : float
        Exploration rate of the Hedge algorithm.
    scale_rewards : callable[[np.ndarray], np.ndarray]
        Function that scales raw rewards into a normalized range (e.g., [0, 1]).
    weights : np.ndarray
        Current Hedge weights for each arm (strategy).
    probs : np.ndarray
    ...
    def __init__(
        ...
        ----------
        epsilon : float
            Exploration parameter of Hedge.
        scale_rewards : callable[[np.ndarray], np.ndarray], default=lambda r: r
            Function to scale raw rewards into a normalized range suitable for Hedge.
    ...
    def compute_epsilon(n_arms: int, n_episodes: int) -> float:
        ...
        The formula used is derived from the theoretical guarantees of the Hedge
